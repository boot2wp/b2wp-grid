import { Icon } from '@wordpress/components';

export const TwoColumnIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="20" y="20" width="26" height="26" />
                <rect x="57" y="20" width="26" height="26" />
                <rect x="57" y="58" width="26" height="26" />
                <rect x="20" y="58" width="26" height="26" />
            </svg>
        }
    />
);

export const ThreeColumnIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="40" y="40" width="21" height="21" />
                <rect x="40" y="12" width="21" height="21" />
                <rect x="40" y="68" width="21" height="21" />
                <rect x="12" y="68" width="21" height="21" />
                <rect x="12" y="40" width="21" height="21" />
                <rect x="12" y="12" width="21" height="21" />
                <rect x="68" y="12" width="21" height="21" />
                <rect x="68" y="40" width="21" height="21" />
                <rect x="68" y="68" width="21" height="21" />
            </svg>
        }
    />
);

export const FourColumnIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="10" y="10" width="17" height="17" />
                <rect x="31" y="10" width="17" height="17" />
                <rect x="52" y="10" width="17" height="17" />
                <rect x="73" y="10" width="17" height="17" />
                <rect x="10" y="31" width="17" height="17" />
                <rect x="31" y="31" width="17" height="17" />
                <rect x="52" y="31" width="17" height="17" />
                <rect x="73" y="31" width="17" height="17" />
                <rect x="10" y="52" width="17" height="17" />
                <rect x="31" y="52" width="17" height="17" />
                <rect x="52" y="52" width="17" height="17" />
                <rect x="73" y="52" width="17" height="17" />
                <rect x="10" y="73" width="17" height="17" />
                <rect x="31" y="73" width="17" height="17" />
                <rect x="52" y="73" width="17" height="17" />
                <rect x="73" y="73" width="17" height="17" />
            </svg>
        }
    />
);

export const TwoColumnFeaturedIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="18" y="15" width="63" height="26" />
                <rect x="18" y="55" width="26" height="26" />
                <rect x="55" y="55" width="26" height="26" />
            </svg>
        }
    />
);

export const TwoColumnMagazineIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="19" y="17" width="26" height="66" />
                <rect x="56" y="17" width="26" height="26" />
                <rect x="56" y="57" width="26" height="26" />
            </svg>
        }
    />
);

export const TwoColumnSidebarIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="65" y="17" width="18" height="66" />
                <rect x="15" y="17" width="41" height="66" />
            </svg>
        }
    />
);

export const TwoColumnEqualIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="17" y="17" width="29" height="66" />
                <rect x="56" y="17" width="29" height="66" />
            </svg>
        }
    />
);

export const AutoColumnIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="14" y="40" width="18" height="26" />
                <rect x="37" y="40" width="26" height="26" />
                <rect x="68" y="40" width="19" height="26" />
            </svg>
        }
    />
);

export const AutoRowIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="12" y="68" width="77" height="21" />
                <rect x="12" y="40" width="77" height="21" />
                <rect x="12" y="12" width="77" height="21" />
            </svg>
        }
    />
);

export const ThreeColumnFeaturedIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="12" y="68" width="49" height="21" />
                <rect x="12" y="40" width="49" height="21" />
                <rect x="12" y="12" width="77" height="21" />
                <rect x="68" y="40" width="21" height="21" />
                <rect x="68" y="68" width="21" height="21" />
            </svg>
        }
    />
);

export const ThreeColumnMagazineIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="40" y="68" width="21" height="21" />
                <rect x="12" y="68" width="21" height="21" />
                <rect x="12" y="12" width="49" height="49" />
                <rect x="68" y="12" width="21" height="21" />
                <rect x="68" y="40" width="21" height="21" />
                <rect x="68" y="68" width="21" height="21" />
            </svg>
        }
    />
);

export const ThreeColumnEqualIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="40" y="40" width="21" height="21" />
                <rect x="40" y="12" width="21" height="21" />
                <rect x="40" y="68" width="21" height="21" />
                <rect x="12" y="68" width="21" height="21" />
                <rect x="12" y="40" width="21" height="21" />
                <rect x="12" y="12" width="21" height="21" />
                <rect x="68" y="12" width="21" height="21" />
                <rect x="68" y="40" width="21" height="21" />
                <rect x="68" y="68" width="21" height="21" />
            </svg>
        }
    />
);

export const FourColumnFeaturedIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="10" y="10" width="80" height="38" />
                <rect x="10" y="52" width="17" height="17" />
                <rect x="31" y="52" width="17" height="17" />
                <rect x="52" y="52" width="17" height="17" />
                <rect x="73" y="52" width="17" height="17" />
                <rect x="10" y="73" width="17" height="17" />
                <rect x="31" y="73" width="17" height="17" />
                <rect x="52" y="73" width="17" height="17" />
                <rect x="73" y="73" width="17" height="17" />
            </svg>
        }
    />
);

export const FourColumnMagazineIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="10" y="10" width="59" height="40" />
                <rect x="73" y="10" width="17" height="17" />
                <rect x="73" y="31" width="17" height="17" />
                <rect x="10" y="52" width="17" height="17" />
                <rect x="31" y="52" width="17" height="17" />
                <rect x="52" y="52" width="17" height="17" />
                <rect x="73" y="52" width="17" height="17" />
                <rect x="10" y="73" width="17" height="17" />
                <rect x="31" y="73" width="17" height="17" />
                <rect x="52" y="73" width="17" height="17" />
                <rect x="73" y="73" width="17" height="17" />
            </svg>
        }
    />
);

export const FourColumnEqualIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="10" y="10" width="17" height="17" />
                <rect x="31" y="10" width="17" height="17" />
                <rect x="52" y="10" width="17" height="17" />
                <rect x="73" y="10" width="17" height="17" />
                <rect x="10" y="31" width="17" height="17" />
                <rect x="31" y="31" width="17" height="17" />
                <rect x="52" y="31" width="17" height="17" />
                <rect x="73" y="31" width="17" height="17" />
                <rect x="10" y="52" width="17" height="17" />
                <rect x="31" y="52" width="17" height="17" />
                <rect x="52" y="52" width="17" height="17" />
                <rect x="73" y="52" width="17" height="17" />
                <rect x="10" y="73" width="17" height="17" />
                <rect x="31" y="73" width="17" height="17" />
                <rect x="52" y="73" width="17" height="17" />
                <rect x="73" y="73" width="17" height="17" />
            </svg>
        }
    />
);

export const CenteredIcon = () => (
    <Icon
        icon={
            <svg width="100" height="100" viewBox="0 0 100 100" >
                <rect x="36" y="37" width="26" height="26" />
                <rect x="9" y="8" width="6" height="84" />
                <rect x="84" y="8" width="6" height="84" />
                <rect x="12" y="8" width="72" height="6" />
                <rect x="13" y="86" width="72" height="6" />
            </svg>
        }
    />
);
